...
def test_owner_recovers_erc20_with_zero_amount(
    rewards_manager, ldo_token, manager_owner, dao_treasury
):
...
def test_owner_recovers_erc20_with_zero_recipient(
    ...
    with reverts("zero address not allowed"):
        rewards_manager.recover_erc20(ldo_token, 0, ZERO_ADDRESS, {"from": manager_owner})


def test_owner_recovers_erc20_with_balance(
    rewards_manager, ldo_token, manager_owner, stranger, dao_treasury
):
    recipient = stranger
    transfer_amount = Wei("1 ether")
    recover_amount = Wei("0.5 ether")
    ldo_token.transfer(rewards_manager, transfer_amount, {"from": dao_treasury})
...
def test_owner_recovers_erc20_to_the_caller_by_default(
    ...
    tx = rewards_manager.recover_erc20(ldo_token, transfer_amount, {"from": manager_owner})
...
def test_recover_erc20_not_enough_balance(rewards_manager, ldo_token, manager_owner, dao_treasury):
    transfer_amount = Wei("1 ether")
    recover_amount = Wei("2 ether")
    ldo_token.transfer(rewards_manager, transfer_amount, {"from": dao_treasury})

    with reverts("ERC20: transfer amount exceeds balance"):
        rewards_manager.recover_erc20(ldo_token, recover_amount, {"from": manager_owner})
